...
public class Diamond {
	...
	// 표시할 줄의 갯수를 받아서 console에 표현.
	public void print2console(int paraInt) {
		
		// 다이아몬드 출력을 위한 변수 선언.
		int iter3, iter4, iter5, iter6;
		// 별이 최대로 표시되는 줄이 몇번째 인지 확인, 이때
		int max = paraInt/2 + 1;
		...
		// 1차 줄바꿈을 위한 for문 - max번째 줄까지 표현
		for (iter3 = 0; iter3 < max; iter3++) {
			// 2차 앞쪽의 공백을 표기하기 위한 for문 - 공백의 갯수가 max-1 ~ 0 으로 감소. 
			for (iter4 = max - 1 - iter3; iter4 > 0; iter4--) {
				System.out.printf(" ");
			}
			// 2차 * 모양을 표시하기 위한 for문 - 앞쪽 *의 갯수가 1 ~ max 로 증가.
			for (iter5 = iter3; iter5 >= 0; iter5--) {
				System.out.printf("*");
			}
			// 2차 * 모양을 표기하기 위한 for문. - 뒤쪽 *의 갯수가 0 ~ max 로 증가.
			for (iter6 = 0; iter6 < iter3; iter6++) {
				System.out.printf("*");
			}
			// 줄바꿈.
		...
		// 1차 줄바꿈을 위한 for문. - max+1 ~ line 줄까지 표현
		for(iter3 = 1; iter3 < max; iter3++) {
			// 2차 앞쪽의 공백을 표기하기 위한 for문. 공백의 갯수가 1 ~ max 으로 증가
			for (iter4 = 0; iter4 < iter3; iter4++) {
				System.out.printf(" ");
			}
			// 2차 * 모양을 표기하기 위한 for문. - 앞쪽 *의 갯수가 max ~ 1로 감소.
			for (iter5 = max; iter5 > iter3; iter5--) {
				System.out.printf("*");
			}
			// 2차 * 모양을 표기하기 위한 for문. - 뒤쪽 *의 갯수가 max-1 ~ 0 으로 감소.
			for (iter6 = max-1; iter6 > iter3; iter6--) {
				System.out.printf("*");
			}
			// 줄바꿈.
			System.out.println("");
